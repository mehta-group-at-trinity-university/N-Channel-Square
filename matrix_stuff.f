...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      ...
      implicit none
      double precision A(N,N),ATEMP(N,N),EVAL(N), EVEC(N,N), norm
      character*1 JOBZ, UPLO
      integer N, LDA, LWORK, INFO, i, ip
      ...
      ATEMP = A                 ! Use a temporary matrix so A is not written over.
      call DSYEV(JOBZ, UPLO, N, ATEMP, LDA, EVAL, WORK, LWORK, INFO)
      LWORK = WORK(1)
      deallocate(WORK)
      allocate(WORK(LWORK))
      call DSYEV(JOBZ, UPLO, N, ATEMP, LDA, EVAL, WORK, LWORK, INFO)
      EVEC = ATEMP
      IF( INFO.GT.0 ) THEN
         WRITE(*,*)'MyDSYEV failed to compute eigenvalues.'
         STOP
      END IF
!     write(6,*) "Done diagonalizing... info = ", info

!     Uncomment the following to check the normalization  
!     (DSYEV should always return normalized vectors)
c$$$      do i=1,N
c$$$         norm=0d0
c$$$         do ip=1,N
c$$$            norm = norm + A(ip,i)**2
c$$$         enddo
c$$$!         write(6,*) "check norm = ", norm
c$$$         EVEC(:,i) = A(:,i)/sqrt(norm) ! use for psi = U.phi project dressed on undressed
c$$$      enddo
